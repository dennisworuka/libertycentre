{{--
    ...
    components/layout/base.blade.php — identical except each hardcoded
    inline <script> now carries @nonce. Filament has no first-party CSP
    nonce support for its own inline scripts (only Livewire's own
    generated tags fall back to Vite::cspNonce(), wired in
    AppServiceProvider), so under our strict script-src 'self'
    'nonce-...' CSP these were silently blocked by the browser — no
    console-visible error from our side, just Alpine/Livewire never
    initialising, which is why the login form looked "not functional".
    Re-diff against the vendor file on any filament/filament upgrade.
--}}
@props([
    'livewire' => null,
])
...
>
    ...
    >
        ...
        @if (filament()->hasBroadcasting() && config('filament.broadcasting.echo'))
            <script data-navigate-once @nonce>
